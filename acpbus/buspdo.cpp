...
)
{
    ...
    DECLARE_CONST_UNICODE_STRING(deviceLocation, L"AMD Audio CoProcessor");
    ...
    if (Desc->CodecIds.IsDSP) {
        ...
        status = RtlUnicodeStringPrintf(&compatId, L"CSAUDIO\\ACP&CLTR_REV_%02X",
            Desc->CodecIds.RevId);
        if (!NT_SUCCESS(status)) {
            return status;
        }
    ...
    ACPDSP_BUS_INTERFACE busInterface = ACPDSP_BusInterface(pdoData);
    WDF_QUERY_INTERFACE_CONFIG_INIT(&qiConfig,
        (PINTERFACE)&busInterface,
        &GUID_ACPDSP_BUS_INTERFACE,
        NULL);
