...
)
{
    ...
    ACPBusPrint(DEBUG_LEVEL_INFO, DBG_INIT,
        "%s\n", __func__);
    ...
    if (!NT_SUCCESS(status)) {
        ACPBusPrint(DEBUG_LEVEL_ERROR, DBG_INIT,
...
)
{
    ...
    if (!NT_SUCCESS(status))
    {
        ACPBusPrint(DEBUG_LEVEL_ERROR, DBG_PNP,
...
)
{
    ...
    fdoCtx->BusInterface.GetBusData(fdoCtx->BusInterface.Context, PCI_WHICHSPACE_CONFIG, &fdoCtx->revId, 8, sizeof(UINT8));

    fdoCtx->sofTplg = NULL;
    fdoCtx->sofTplgSz = 0;

    { //Check topology for AMD SOF
        SOF_TPLG sofTplg = { 0 };
        NTSTATUS status2 = GetSOFTplg(Device, &sofTplg);
        if (NT_SUCCESS(status2) && sofTplg.magic == SOFTPLG_MAGIC) {
            fdoCtx->sofTplg = ExAllocatePoolZero(NonPagedPool, sofTplg.length, ACPBUS_POOL_TAG);
...
)
{
    ...
    if (fdoCtx->sofTplg)
        ExFreePoolWithTag(fdoCtx->sofTplg, ACPBUS_POOL_TAG);
...
)
{
    ...
    description.CodecIds.RevId = fdoCtx->revId;
